...
export default {
  ...
  methods: {
    ...
    getChartData() {
      ...
      _this = this;
      this.$http.get('/api/getChartData', this.parseToSend({ pair: this.currentPair.code })).then(
        response => {
    ...
    createFeed() {
              ...
              that._datafeed.getBars(
                ...
                function(bars) {
                  ...
                  if (isNewBar) {
                    ...
                    for (const i = 0; i < subscribers.length; ++i) {
      ...
      Datafeed.Container.prototype._fireEvent = function(event, argument) {
        if (this._callbacks.hasOwnProperty(event)) {
          ...
          for (const i = 0; i < callbacksChain.length; ++i) {
    ...
    adjustChart() {
      const chartIframe = $('#chart_container').find('iframe');
      const chartTop = chartIframe.contents().find('.header-chart-panel');
      const chartTopContainer = chartIframe.contents().find('.header-chart-panel-content');
      const chartBottom = chartIframe.contents().find('.date-range-wrapper');

      chartBottom.appendTo(chartTopContainer);
    },
  },
  watch: {
    pair(newVal, oldVal) {
